...
paper's extracted fields (title, authors, journal, electrolyte, salt, solvent, additive, \
cathode, anode, separator, cell_type, voltage_window, temperature, formation_protocol, \
cycle_condition, rate_capability, main_findings, innovation, advantages, limitations, \
future_work). Some fields may be null or empty where the original paper didn't report them.
...
- comparison_table: exactly one row per input paper, in the same order given, with \
that paper's title and key experimental fields (electrolyte, cathode, anode, separator, \
cell_type, voltage_window, temperature) so they can be scanned side by side.
